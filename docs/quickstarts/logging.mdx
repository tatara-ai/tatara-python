...
from tatara.tatara_types import ImageFormat
...
def generate_character_image(character_visual_description: str, openai_client: OpenAI):
    ...
    tatara.current_span().log_diffusion_success_with_image_data(img_data, ImageFormat.PNG, prompt=character_visual_description)
    return img_data
```
